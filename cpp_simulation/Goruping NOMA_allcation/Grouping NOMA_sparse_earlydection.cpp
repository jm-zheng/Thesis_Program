...
int main()
{
    ...
    D2D_request_file.open("with_earlydection\\"+directory_nMTCD+"\\with_earlydection_D2D_request.csv", fstream::out);
	...
	for( int Now_RAO=0; Now_RAO<simRAo; Now_RAO++)
	{
		...
		for(int i=0; i<nMTCD;i++ )
		{
			...
			if(MTCD_Table[i].D2D_initate_request_RAO == Now_RAO   && MTCD_Table[i].MTCD_RA_status == "D2D_RA"&& MTCD_Table[i].nTransmit_RA < MAX_nTransmit_RA )
			{
				...
				{
                    MTCD_Table[i].D2D_request_allocation_index= D2D_eachgroup_request_time[MTCD_Table[i].group];
...
//--------確認發起RA後的狀態----------------------------------------------------------------------------------------
    ...
    nMTCD_file.open("with_earlydection\\"+directory_nMTCD+"\\with_earlydection_RAtime.csv", fstream::out);
    if (nMTCD_file.is_open())
	{
        nMTCD_file <<"MTCD_number,group,D2D_first_request_RAO,D2D_initate_request_RAO,nRequest_D2D,";
        nMTCD_file <<"RA_first_RAO,RA_initate_RAO,RA_success_RAO,nTransmit_RA,Preamble_number,Power_level,D2D_request_allocation_index "<<endl ;
    ...
    PreStatus_file.open("with_earlydection\\"+directory_nMTCD+"\\with_earlydection_PreStatus.csv", fstream::out);
    ...
    SuccessMTCD_cumulation_file.open("with_earlydection\\"+directory_nMTCD+"\\with_earlydection_SuccessMTCD.csv", fstream::out);
