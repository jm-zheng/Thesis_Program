...
//-------¤À°tgrant------------------------------------------------------------------
		...
		for (int c = 1; c < 55; ++c) {
			...
			else if (eachRAoPre[c] <= 3 && eachRAoPre[c] !=0) {   // ¸Ó­Ópreamble³Ì¦h3­Ó³]³Æµoþ
	...
	double nMTCDs =nMTCD;  //ÅýÅÜ¼Æ=±`¼Æ°µ³B²z
	file1.close();
	cout <<"success"<< SuccessnMTCD <<" fail: "<<numMTCDfail << endl;
	cout << "Average Access Delay:" << (double(totaldelay) / double(SuccessnMTCD) /100)+0.016 << endl;//°£¥H100±M´«¦¨¬í
