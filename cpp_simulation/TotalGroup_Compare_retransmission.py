...
Axis_index = 0 # 0:50k 1:100K 2:150K
bar_width = 0.15  # 有四項數據
bar_width_align=bar_width/2

Group_nTransmit_Yaxis=[11000,30000,50000,60000,80000]
...
plt.bar(np.arange(1,8)+(bar_width*1-bar_width_align), Uniform_Preamble_allocation_nTransmit_RA,width=bar_width,color="r",label="Preamble allocationDense(Uniform)")
plt.bar(np.arange(1,9)+(bar_width*2-bar_width_align), Dense_Preamble_allocationDense_nTransmit_RA,width=bar_width,color="tomato",label="Preamble allocationDense(Dense)")

plt.legend(loc='upper center',fontsize=16)
plt.ylim(bottom=0)
plt.xlim(left=0.5)
plt.title('MTCDs transmission times ( nMTCD='+ dircectory_nMTCD+", r=10) ",fontsize=26)
