...
double each_PACB[2][simRAo]={0} ; //each:PACB
...
//ªì©l¤Æ
  ...
  double SIB2_ACBP=0;
	...
	for (int a = 0; a < simRAo; ++a) {
		...
		if (initiatedRA < 54) { SIB2_ACBP += 1; }
		else { SIB2_ACBP = SIB2_ACBP+(static_cast < double>(54) / initiatedRA); }



		each_PACB[1][a] =initiatedRA;  //Á`¦@¦³¦h¤Ö³]³Æ
		int eachRAoPre[55] = { 0 };
		...
		if(used_Pre>30) used_Pre=30; //¯à¨Ï¥Îªºgrant¶Ç°eªºpre³Ì¦h´N¬O30­Ó 10*3
		...
		/*for (int c = 1; c < 55; ++c) {
			...
			else if (eachRAoPre[c] == 1) {   // ¸Ó­Ópreamble¥u¦³1­Ó³]³Æµo??
				++PreStatus[3][a];
				++SuccessnMTCD;
				SuccessnMTCDslot[a]++;
				for (int d = 0; d < nMTCD; ++d) {
	...
	for(int i=0;i<simRAo;i++)
	{
		...
		file1 <<PreStatus[0][i]<<","<<PreStatus[1][i]<<","<<PreStatus[2][i]<<","<<PreStatus[3][i]
			  << ","<<PreStatus[4][i]<<","<<(static_cast<double>(PreStatus[2][i])/54)<<","<<each_PACB[0][i]<<","<<each_PACB[1][i]<<endl;
	}
	file1.close();
