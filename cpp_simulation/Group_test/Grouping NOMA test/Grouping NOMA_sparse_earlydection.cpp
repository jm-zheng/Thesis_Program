...
const int SIB2_cycle =8; //D2D_cycle 80ms =8 RAO
	...
	for( int Now_RAO=0; Now_RAO<simRAo; Now_RAO++)
	{
		...
		for(int i=0; i<nMTCD;i++ )
		{
			...
			if(MTCD_Table.at(i).D2D_initate_request_RAO == Now_RAO   && MTCD_Table.at(i).MTCD_RA_status == "D2D_init"&& MTCD_Table.at(i).nTransmit_RA < MAX_nTransmit_RA )
			{
				...
				if((Now_RAO % D2D_cycle) <4 && D2D_eachgroup_request_time[MTCD_Table.at(i).group] < D2DRB_Pool) // D2D 沒有超過RB pool
				{
                    ...
                    /*if(MTCD_Table[i].group <26) //組別小於26
                    {
                        if(D2D_eachgroup_request_time[MTCD_Table[i].group] < D2D_cycle)  //D2D_cycle次數
                        {
                            //紀錄head分配第一次RA的RAO
                            if(MTCD_Table[i].RA_first_RAO == 0)MTCD_Table[i].RA_first_RAO = D2D_1th_request_EachCycle + D2D_eachgroup_request_time[MTCD_Table[i].group] + D2D_cycle;
                            MTCD_Table[i].RA_initate_RAO = D2D_1th_request_EachCycle + D2D_eachgroup_request_time[MTCD_Table[i].group] + D2D_cycle;	//紀錄head分配每次RA的RAO ex: 0+(total request-1)1+8 =9 下次D2D週期第2個RAO發起
                            MTCD_Table[i].Preamble_number = MTCD_Table[i].group;
                            MTCD_Table[i].Power_level =0;
                        }
                        else  //大於D2D_cycle次數
                        {
                            if(D2D_eachgroup_request_time[MTCD_Table[i].group]< Threshold_PowerAllocation )  //在index 16 或19的演算法
                            {
                                if(MTCD_Table[i].RA_first_RAO == 0)MTCD_Table[i].RA_first_RAO = D2D_1th_request_EachCycle + (D2D_eachgroup_request_time[MTCD_Table[i].group]%4) + D2D_cycle;
                                    MTCD_Table[i].RA_initate_RAO = D2D_1th_request_EachCycle +(D2D_eachgroup_request_time[MTCD_Table[i].group]%4) + D2D_cycle;	//紀錄head分配每次RA的RAO ex: 0+(total request % 4)1+8 =9 下次D2D週期第2個RAO發起
                                    MTCD_Table[i].Preamble_number = MTCD_Table[i].group;
                                    MTCD_Table[i].Power_level =(D2D_eachgroup_request_time[MTCD_Table[i].group]/4);
                            }
                            else
                            {
                                    if(MTCD_Table[i].RA_first_RAO == 0)MTCD_Table[i].RA_first_RAO = D2D_1th_request_EachCycle + ((D2D_eachgroup_request_time[MTCD_Table[i].group]%Threshold_PowerAllocation%4)+4) + D2D_cycle;
                                    MTCD_Table[i].RA_initate_RAO = D2D_1th_request_EachCycle +((D2D_eachgroup_request_time[MTCD_Table[i].group]%Threshold_PowerAllocation)+4) + D2D_cycle;	//紀錄head分配每次RA的RAO ex: 0+(total request %4 + 4)1+8 =9 下次D2D週期第2個RAO發起
                                    MTCD_Table[i].Preamble_number = MTCD_Table[i].group;
                                    MTCD_Table[i].Power_level =((D2D_eachgroup_request_time[MTCD_Table[i].group]%Threshold_PowerAllocation)/4);
                            }
                        }
                        MTCD_Table[i].MTCD_RA_status ="RA";
                    }
                    else //組別大於26
                    {
                        if(D2D_eachgroup_request_time[MTCD_Table[i].group] < D2D_cycle)  //D2D_cycle次數
                        {
                            //紀錄head分配第一次RA的RAO
                            if(MTCD_Table[i].RA_first_RAO == 0)MTCD_Table[i].RA_first_RAO = D2D_1th_request_EachCycle + (D2D_cycle -1 - D2D_eachgroup_request_time[MTCD_Table[i].group]%4) + D2D_cycle;  //從26group開始從D2D週期的最後一個RA開始分配
                            MTCD_Table[i].RA_initate_RAO = MTCD_Table[i].RA_first_RAO = D2D_1th_request_EachCycle + (D2D_cycle -1 - D2D_eachgroup_request_time[MTCD_Table[i].group]%4) + D2D_cycle;	//紀錄head分配每次RA的RAO ex: 0+(total request-1)1+8 =9 下次D2D週期第2個RAO發起
                            MTCD_Table[i].Preamble_number = MTCD_Table[i].group;
                            MTCD_Table[i].Power_level =0;
                             MTCD_Table[i].MTCD_RA_status ="RA";
                        }
                        else  //大於D2D_cycle次數
                        {
                            if(D2D_eachgroup_request_time[MTCD_Table[i].group]< Threshold_PowerAllocation )  //在index 16 或19的演算法
                            {
                                if(MTCD_Table[i].RA_first_RAO == 0)MTCD_Table[i].RA_first_RAO = D2D_1th_request_EachCycle + D2D_cycle -1 -(D2D_eachgroup_request_time[MTCD_Table[i].group]%4) + D2D_cycle;  // D2D週期起始RAO + (從第7個RAO)D2D週期的最後一個RA開始分配 + 下個D2D週期的時候發
                                    MTCD_Table[i].RA_initate_RAO = D2D_1th_request_EachCycle + (D2D_cycle -1 -D2D_eachgroup_request_time[MTCD_Table[i].group]%4) + D2D_cycle;	//紀錄head分配每次RA的RAO ex: 0+(total request % 4)1+8 =9 下次D2D週期第2個RAO發起
                                    MTCD_Table[i].Preamble_number = MTCD_Table[i].group;
                                    MTCD_Table[i].Power_level =(D2D_eachgroup_request_time[MTCD_Table[i].group]/4);
                                    MTCD_Table[i].MTCD_RA_status ="RA";
                            }
                            ...
                            {
                                    if(MTCD_Table[i].RA_first_RAO == 0)MTCD_Table[i].RA_first_RAO = D2D_1th_request_EachCycle + (D2D_cycle -5 -(D2D_eachgroup_request_time[MTCD_Table[i].group]%Threshold_PowerAllocation)%4) + D2D_cycle; // D2D週期起始RAO + (從第3個RAO開始)D2D週期的最後一個RA開始分配 + 下個D2D週期的時候發
                                    MTCD_Table[i].RA_initate_RAO = D2D_1th_request_EachCycle + (D2D_cycle -5 -(D2D_eachgroup_request_time[MTCD_Table[i].group]%Threshold_PowerAllocation)%4) + D2D_cycle;	//紀錄head分配每次RA的RAO ex: 0+(total request %4 + 4)1+8 =9 下次D2D週期第2個RAO發起
                                    MTCD_Table[i].Preamble_number = MTCD_Table[i].group;
                                    MTCD_Table[i].Power_level =((D2D_eachgroup_request_time[MTCD_Table[i].group]%Threshold_PowerAllocation)/4);
				...
				else //if(Now_RAO % D2D_cycle >=4 && D2D_eachgroup_request_time[MTCD_Table.at(i).group] > D2DRB_Pool)  //D2D request fail
				{
					if(MTCD_Table.at(i).nRequest_D2D < MAX_nRequest_D2D) MTCD_Table.at(i).D2D_initate_request_RAO = Now_RAO+4; //D2D backoff
					else
					{
							//使用share preamble 先丟到 SharePre_OutsidePool 在此次D2D週期結束才會釋放到 SharePre_RA_InsidePool
							MTCD_Table.at(i).MTCD_RA_status = "SharePre_OutsidePool";
							MTCD_Table.at(i).RA_initate_RAO = D2D_1th_request_EachCycle +8; // 在下一個D2D釋放出去
					}
				}

			}
			// 使用share preamble RA的設備
			if(MTCD_Table.at(i).MTCD_RA_status == "SharePre_RA_InsidePool" && MTCD_Table.at(i).RA_initate_RAO == Now_RAO&& MTCD_Table.at(i).nTransmit_RA < MAX_nTransmit_RA  )
			{
				MTCD_Table.at(i).P_sharePre_access = (double)rand()/(RAND_MAX+1);
...
//----------------分配grant-----------------------------------------------------------------------------------------------------------
		...
		for(int i=0; i< nMTCD ; i++)
		{
			if(MTCD_Table.at(i).RA_initate_RAO == Now_RAO && MTCD_Table.at(i).nTransmit_RA < MAX_nTransmit_RA )
			{
				each_RAO_Premble[MTCD_Table.at(i).Preamble_number]+=1;
...
//--------確認發起RA後的狀態----------------------------------------------------------------------------------------
            ...
            else
            {
                for(int i=0; i<nMTCD ; i++)
                {
                    if(MTCD_Table.at(i).RA_initate_RAO == Now_RAO && MTCD_Table.at(i).Preamble_number == each_RAO_Pre_index )
                    {
                        ...
                        {
                            collision_Share_Preamble_it = find(collision_Share_Preamble.begin(),collision_Share_Preamble.end(),MTCD_Table.at(i).Preamble_number);
                            ...
                            if(MTCD_Table.at(i).nTransmit_RA<10)
                            {
                                MTCD_Table.at(i).D2D_initate_request_RAO = Now_RAO +(Backoff_RA / 10);
                                MTCD_Table.at(i).MTCD_RA_status = "D2D_init";
                            }
                            else
                            {
                        ...
                        else if (each_RAO_Premble[each_RAO_Pre_index] <= 3 && MTCD_Table.at(i).Preamble_number <= nGroup)  //Group裡面分配成功的
                        {
                            bool grant_check_index =0;

                            for(int x=1 ; x<=used_Pre;x++ )
                            {
                                //  cout<<"Preamble:"<<MTCD_Table.at(i).Preamble_number<<"  vs  " <<eachRAOPre_transmit_shuffle[x]<<endl;
                                if (MTCD_Table.at(i).Preamble_number == eachRAOPre_transmit_shuffle[x] )
                                {
                                    grant_check_index =1;
                        ...
                        else if (each_RAO_Premble[each_RAO_Pre_index] == 1 && MTCD_Table.at(i).Preamble_number > nGroup)  // 使用sharePreamble 成功
                        {
                            bool grant_check_index =0;
    ...
    nMTCD_file.open("Uniform\\"+directory_ACB+"\\"+directory_nMTCD+"\\"+directory_ACB+"with_earlydection_RAtime.csv", fstream::out);
    if (nMTCD_file.is_open())
	{
        nMTCD_file <<"MTCD_number,group,D2D_first_request_RAO,D2D_initate_request_RAO,nRequest_D2D,";
    ...
    PreStatus_file.open("Uniform\\"+directory_ACB+"\\"+directory_nMTCD+"\\"+directory_ACB+"with_earlydection_PreStatus.csv", fstream::out);
    ...
    SuccessMTCD_cumulation_file.open("Uniform\\"+directory_ACB+"\\"+directory_nMTCD+"\\"+directory_ACB+"with_earlydection_SuccessMTCD.csv", fstream::out);
    ...
    D2D_request_file.open("Uniform\\"+directory_ACB+"\\"+directory_nMTCD+"\\"+directory_ACB+"with_earlydection_D2D_request.csv", fstream::out);
	...
	fstream result_file;
	result_file.open("Uniform\\"+directory_ACB+"\\"+directory_nMTCD+"\\"+directory_ACB+"with_earlydection_result.txt",fstream :: out);
