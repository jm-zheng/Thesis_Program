...
colormap = np.array(['chartreuse', 'aqua','burlywood','cadetblue','chocolate','cornflowerblue','rosybrown','crimson','blue','brown',
    ...
    if nhx == list(hx) and nhy == (hy):
        ...
        for index, nodes in enumerate(team):
            for i in range(len(nodes)):
                plt.scatter(nodes[i][0],nodes[i][1],c=colormap[index],s=5)#s:# 點size c:顏色
